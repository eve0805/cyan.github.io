...
凭据事件已经接入了 `Krb5CcachePresenter`。当代码路径里真正提取出了 TGT、TGS 或 Delegation TGS 对应的 credential 时，logger 能给出相对完整的 ccache 风格摘要。
...
**AS-REQ and KRB-ERROR (pre-authentication required)。** 初始的 AS 交换里，KDC 返回需要补充预认证信息的错误，这张图主要用来说明 trace 已经能把预认证挑战清楚打出来。

![AS-REQ and AS-REP](../../pic/AS-REQ%20and%20AS-REP.png)

**AS-REQ and AS-REP。** 补齐预认证信息之后，请求进入成功的 AS 交换阶段，可以看到完整的请求和响应输出。

![TGT](../../pic/TGT.png)

**TGT。** 成功完成 AS 流程之后，trace 会把提取出来的 TGT 凭据摘要打印出来。

![TGS-REQ and TGS-REP 1](../../pic/TGS-REQ%20and%20TGS-REP%201.png)

**TGS-REQ and TGS-REP。** 第一段 TGS 交换，重点是展示服务票据请求和响应是怎样被 trace 记录下来的。

![TGS](../../pic/TGS.png)

**TGS。** 这张图对应 TGS 交换完成后提取出的服务票据凭据摘要。

![TGS-REQ and TGS-REP 2](../../pic/TGS-REQ%20and%20TGS-REP%202.png)

**TGS-REQ and TGS-REP。** 另一段 TGS 交换示例，用来说明 trace 会继续覆盖后续的服务票据活动，而不只停留在第一次请求上。

![Delegation TGS](../../pic/Delegation%20TGS.png)

**Delegation TGS。** 这一张对应 delegation 场景下产出的票据凭据，说明凭据类事件已经能延伸到更具体的 Kerberos 使用路径。
...
其实开始的时候是只有`msg_type:30`这样的效果的，但是 mentor 提了一个建议：
...
在做的是 Trace 的功能所以要考虑人类可读性，所以又新改了输出格式，确实美观和清晰了很多【应该大概也许？】。
